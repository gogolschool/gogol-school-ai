...
3. **Run fetch** (only on fetch intent):
   ```bash
   python3 ~/.claude/skills/payments-reconcile/scripts/fetch_payments.py <YYYY-MM-DD>
   ```
...
5. **Run reconcile**:
   ```bash
   python3 ~/.claude/skills/payments-reconcile/scripts/reconcile.py /tmp/reconcile_<date>/
   ```
...
```bash
cp ~/.claude/skills/payments-reconcile/.env.example ~/.claude/skills/payments-reconcile/.env
