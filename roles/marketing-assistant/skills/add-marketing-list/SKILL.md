...
- **`funql_query` отдаёт МАКСИМУМ 50 строк за вызов** (жёсткий лимит MCP: `_returned:50`, `_truncated:true`, даже при `LIMIT 200`). Одним запросом все id НЕ получить. Поэтому для сегмента >50 человек:
  - **Предпочтительно** — не собирать id руками, а гнать через `marketing.create_list_from_userview_elements` (Озма сама прокручивает вью батчами по 300 на сервере, id вообще не трогаешь). Нужен вью с этим фильтром — проверь `list_user_views`; если подходящего нет, можно один раз завести (или попросить создать) параметризованный вью под сегмент.
  - **Фолбэк** (если вью нет) — пагинация `offset`-ом с шагом 50 (`_total` = общий счётчик), собрать все id, порезать на чанки по **≤500** и вызвать `create_list_from_selected_elements` по каждому чанку. Медленно (напр. 1111 чел. = 23 запроса), но работает.
  - Cap 50 для preview человеку — это норм; для заливки в action нужны ВСЕ id (см. выше).
