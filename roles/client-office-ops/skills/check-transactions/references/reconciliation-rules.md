...
- **Свежесть подтверждения.** `provider_confirmed_at(tx)` берёт epoch-поля CP (`ConfirmDate`, затем `AuthDate`) — только они однозначны, потому что `*Iso`-поля CP отдаёт в МСК, а нормализованный `datetime_utc` у CP тоже МСК (без смещения), у Mixplat — со смещением. Если подтверждение моложе `FRESH_CONFIRM_WINDOW` (60 мин) от момента выгрузки, `stuck_pending` помечается `fresh=true` → 🟡 «вебхук мог не успеть», а не 🔴.
