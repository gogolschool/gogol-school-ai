...
- У провайдера по этому id нет денег (`failed`/`pending`/строки нет) → **`pending_ignored`**: брошенная попытка, в матчинге не участвует, но id перечисляются одной info-строкой (ничего не теряется молча).
