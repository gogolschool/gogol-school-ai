...
- **`/b2b`** — раздел «Корпоративные клиенты»: завести клиента, контакт, сделку, продукт из сделки, действие, дописать ретеншн и итоги, разобрать заявку с b2b-сайта, ответить по отчётам; понимает голосовые, «это Вика» переключает на аккаунта (см. [skills/b2b](skills/b2b/SKILL.md)). Шаги цепочки сделки — `/b2b-deals` (Дамблдор).
