...
5. **Корпоративные клиенты (B2B)** — клиенты, сделки, продукты из сделок, действия и отчёты раздела «Корпоративные клиенты» в OzmaDB.
...
- **`/b2b`** — ассистент B2B-продавца в разделе «Корпоративные клиенты»: заводит клиента, контакт, сделку, продукт из сделки, действие (дозадаёт минимум вопросов, по одному, запись после «да»), дописывает ретеншн и итоги, разбирает заявки с b2b-сайта, коротко отвечает по отчётам и напоминает вернуться к клиенту; понимает голосовые, «это Вика» переключает на аккаунта (см. [skills/b2b](skills/b2b/SKILL.md)). Шаги цепочки сделки — `/b2b-deals` (Джангир).

## С какими системами работаю

- **OzmaDB** (основное): схемы `crm` (продукты, заявки, занятия, мастера, B2B: `b2b_clients`, `b2b_deals`, `b2b_deal_actions`), `staffs`/`analytics` (ЗП и отметки), `base`
