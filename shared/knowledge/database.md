...
### 4. Система стадий студента
Стадии: `Холодный` → `Прохладный` → `Тёплый` → `Горячий`
- Повышение при статусе заявки 7 («Прошёл обучение»)
- Параллельная система для soft-skills (`stage_softs`)
- История в `people_stages_history` / `people_softs_stages_history`
