...
- **`action_datetime`** — дата и время старта (у продукта = `MIN(action_datetime)` дочерних «Занятие»), **`end_datetime`** — окончание. Хранятся в UTC, МСК = +3 часа. Это единственный источник ответа на вопрос «когда продукт начинается».
- **`pl_start_date` / `pl_end_date`** — период признания выручки в P&L (`total_month_count`, раскладка `fin.pl_report_final` по месяцам), **не даты проведения**. У открытых уроков там стоят даты лаборатории, к которой ведёт ОУ; у части продуктов поля пустые. Фильтровать по ним «стартующие продукты» нельзя — молча теряются строки.
